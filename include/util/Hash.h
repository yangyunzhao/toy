...
requires std::is_enum_v<Enum> struct hash<Enum> {
    using is_avalanching = void;
...
private:
    static constexpr size_t alignUp(size_t n) noexcept {
...
public:
    ...
    StackAllocator(Storage& storage) noexcept
        : storage(storage) {
        static_assert(N % Align == 0, "size N needs to be a multiple of alignment Align");
    }

    template <typename U>
    StackAllocator(const StackAllocator<U, N, Align>& other) noexcept
...
template <typename K, typename V, typename H = hash<K>, typename E = std::equal_to<K>, typename A = std::allocator<std::pair<const K, V>>>
using flat_hash_map = boost::unordered_flat_map<K, V, H, E, A>;

template <typename T, typename H = hash<T>, typename E = std::equal_to<T>, typename A = std::allocator<T>>
